...
consl();
const mytitles = document.querySelector(".titles");
mytitle.forEach((titl, num) => {
  mytitles.innerHTML += `
      <li class="col-span-1  ">
        <a href="${titl.link}" class="flex  flex-col lg:flex-row items-center p-0 text-[#ccc] no-underline hover:text-[#e10800]">
          ${titl.icon}
          ${titl.title}
        </a>
      </li>
...
const titls = [
  ...
  },
  {
    ...
    title: "Muose",
    link2: "compo.html",
    image2: "./img/compo.webp",
    title2: "Compos",
  },
  {
    link: "HEADSET.html",
    image: "img/headsets.avif",
    title: "Headsets",
    link2: "GAMEPAD.html",
    image2: "img/GAMEPAD-1.webp",
    title2: "GAMEPAD",
  },
  {
    link: "case.html",
...
const mytitleis = document.querySelector(".mydivisrow");
titls.forEach((titl, num) => {

  if (titl === titls[0]) {

    mytitleis.innerHTML += `
  <a class="mydivs flex flex-col items-center justify-center gap-3 mydivs2 KandC bg-[#1e1e1e] " href="${titl.link}">
                    <div class="img1 py-3">
                        <img src="${titl.image}" alt="">
                        <h4>${titl.title}</h4>
                    </div>
                </a>              
      `;
  } else if (titl === titls[3]) {
  ...
  <a class="mydivs flex flex-col items-center justify-center gap-3 mydivs2 KandC bg-[#1e1e1e] " href="${titl.link}">
    ...
    mytitleis.innerHTML += `
  <div class="mydivs flex flex-col gap-3">
                    <a class="img2 mydivs2" href="${titl.link}">
              <div class="img1 py-3">
                            <img src="${titl.image}" alt="">
                            <h4>${titl.title}</h4>
                    ...
                    <a  class="img2 mydivs2" href="${titl.link2}">
                <div class="img1 py-3">
                            <img class="w-[60%] h-[60%]" src="${titl.image2}" alt="">
                            <h4 class="">${titl.title2}</h4>
                </div>
                    </a>
 </div>`;
  }
